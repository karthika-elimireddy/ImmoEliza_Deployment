...
buildingstates = ['AS_NEW', 'JUST_RENOVATED', 'GOOD', 'TO_BE_DONE_UP', 'TO_RENOVATE','TO_RESTORE']
...
with st.form(key='prediction_form'):
    property_type = st.radio('Property Type', options=[1, 0], format_func=lambda x: 'HOUSE' if x == 1 else 'APARTMENT')
    locality = st.selectbox('Select Locality', options=localities)
    mobib_score = st.number_input('Mobility Score', min_value=0.0, max_value=10.0, step=0.1)
    ...
    fireplace = st.radio('Fireplace Available', options=[0, 1], format_func=lambda x: 'Yes' if x == 1 else 'No')
    pool = st.radio('Swimming Pool Available', options=[0, 1], format_func=lambda x: 'Yes' if x == 1 else 'No')
