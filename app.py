...
st.markdown(
    "<h2 style='color: orange;'>Immo Eliza Property Price Estimation</h2>",
    unsafe_allow_html=True,
)

if st.session_state.page == 1:
    st.markdown(
        "<h2 style='color: black;'>Property Information</h2>", unsafe_allow_html=True
    )
    st.markdown(
...
if st.session_state.page == 2:
    st.markdown(
        "<h2 style='color: black;'>Additional Information</h2>", unsafe_allow_html=True
    )
    livingArea = st.number_input(
        "Living Area (sqm) *",
        min_value=0.0,
        step=1.0,
        help="ℹ️ The living area refers to the part of a property that is habitable and used for day-to-day living. It is the portion of the home that is enclosed and usable for living purposes",
    )
...
if st.session_state.page == 3:
    st.markdown(
        "<h2 style='color: black;'>Additional Information (Optional)</h2>",
        unsafe_allow_html=True,
    )
    ...
    mobib_score = st.number_input(
        ...
        max_value=10.0,
        step=0.1,
        help="ℹ️ Choose a mobib score number between 0,0 and 10,0.",
    )

    cadastralIncome = st.number_input(
        "Cadastral Income",
        min_value=0.0,
        step=1.0,
        help="ℹ️ This income can include rent, leasing income, or any other financial benefit derived from the ownership or use of the property as determined by its cadastral assessment.",
    )
...
if st.session_state.page == 4:
    st.markdown(
        "<h2 style='color: black;'>Estimated Property Price</h2>",
        unsafe_allow_html=True,
    )
    st.success(
        f"The estimated price of the property is: €{st.session_state.predicted_price:,.2f}"
    )
